...
These calculations reveal that a nonlinear instability dominates the injection of energy into the turbulence by overtaking the linear drift wave instability that resides in the plasma model.
...
phase shift that causes instability. The turbulence characteristics in the simulations agree remarkably well with experiment. However, when the nonlinear instability is artificially 
removed from the system, the turbulence develops a coherent frequency and wavenumber spectrum, which is inconsistent
with experimental data. This indicates that the nonlinear instability drives the plasma turbulence in the experiment.
\end{abstract}
...
given strong enough finite-amplitude perturbations. Subcritical turbulence is common in hydrodynamics \cite{manneville2008}, and although not as well-known in plasma physics,
...
In these equations, $\mu_N$, $\mu_T$, and $\mu_\phi$ are artificial diffusion and viscosity coefficients used for sub-grid dissipation. They are large enough to cause saturation
and grid convergence, but small enough to allow for turbulence to develop. In the simulations, they are all given the same value of $1.25 \times 10^{-3}$ in their Bohm-normalized units. 
...
The azimuthal average of the potential $\phi$ is allowed to form and evolve, which produces zonal flows.
...
Other possible nonlinearities that do not conserve energy are not included in the model equation set or in simulations for simplicity. 
...
instability, nor a potential-driven flute-like instability such as Kelvin-Helmholtz or interchange. Rather, the cause is a primary nonlinear instability like that described in \cite{biskamp1995,drake1995}. 
...
Fig. \ref{dEdt_tot_fig}a, which is a plot of the nonconservative rate of change of the total energy, $ \pdiff{E(\vec{k})}{t} \big|_{NC} = \sum_j Q_j(\vec{k}) + C_j(\vec{k}) + D_j(\vec{k})$, 
...
are shown in Fig. \ref{gamma_no_n0_figs}. Interestingly, the turbulent growth rate spectrum $\gamma_T(\vec{k})$ is nearly identical to the linear growth rate spectrum $\gamma_T(\vec{k})$, as seen in
...
dominated simulation. While the standard simulation with the $n=0$ modes is quite consistent with experimental data, 
the clear peak in the spectra of the simulation with no $n=0$ inconsistent with experiment, indicating that something in the standard simulation -- possibly
